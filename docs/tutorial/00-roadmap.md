...
- **主要代码**：[agent.ts `stream` / `_think`](../../src/agent/agent.ts#L140-L205)、[agent-event.ts](../../src/agent/agent-event.ts)、[agent/index.ts](../../src/agent/index.ts)
...
- **主要代码**：[agent.ts `_act`](../../src/agent/agent.ts#L222-L272)
...
- **亮点预告**：`convertToOpenAIMessages` 的双向翻译；`StreamAccumulator` 增量拼接**未完成的 tool-call JSON**，并「参数没解析成功前不吐出 tool_use」的严谨；`temperature: 0` 默认值决策。
